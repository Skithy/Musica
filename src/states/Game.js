...
//    this.load.spritesheet('note', 'assets/images/note.png', 39, 39)
  ...
  create () {
    ...
    this.testBox = game.add.graphics(this.world.width + OFFScreen, 0)
    ...
    // Incoming notes
    this.playBox = game.add.graphics(30,30)//this.world.width, 0) //+ OFFScreen, 0)    
    this.playBox.lineStyle(10, 0x000234, 1)
    this.playBox.beginFill(0x00ff54)
    let playSpawn = 0


    // Determines position of incoming boxes and draws them
    for (let i = 0; i < arrSize; i++) {
      const height = this.arrayConvert(arr, arrSize, i) 
      this.playBox.drawRect(playSpawn, height, BOX_SIZE, BOX_SIZE)
      playSpawn += 0//BOX_SIZE
    }
  ...
  arrayConvert (songArr, beatMax, currTime) {
    const note = songArr[currTime % beatMax]
    const dict = this.notesdata[note]
    return dict.pos
  }
